...
class ContentPage(Page):
    ...
    def content_builder(self, s):
        ...
        for i in range(len(s.power_stats)):
            if(i==0):
                self._content += '''<div class="stat" id="power" style="width:''' + str(s.power_stats[i]  - 15) + '''%;">Power</div><div class="percentage">''' + str(s.power_stats[i]) + '''%</div><br>'''
            if(i==1):
                self._content += '''<div class="stat" id="armor" style="width:''' + str(s.power_stats[i]  - 15) + '''%;">Armor</div><div class="percentage">''' + str(s.power_stats[i]) + '''%</div><br>'''
            if(i==2):
                self._content += '''<div class="stat" id="agility" style="width:''' + str(s.power_stats[i]  - 15) + '''%;">Agility</div><div class="percentage">''' + str(s.power_stats[i]) + '''%</div><br>'''
            if(i==3):
                self._content += '''<div class="stat" id="luck" style="width:''' + str(s.power_stats[i]  - 15) + '''%;">Luck</div><div class="percentage">''' + str(s.power_stats[i]) + '''%</div><br>'''
